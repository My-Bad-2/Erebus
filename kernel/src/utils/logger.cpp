...
drivers::uart::SerialPort &get_debug_console(const void *spcr_raw) noexcept {
  ...
  if (const auto *spcr = static_cast<const acpi_spcr *>(spcr_raw);
      spcr != nullptr && !s_acpi_configured) [[unlikely]] {
    const auto type = (spcr->base_address.address_space_id == 0)
                          ? hw::IoType::MMIO
                          : hw::IoType::PMIO;
    const std::uint8_t stride = (spcr->base_address.access_size > 0)
                                    ? spcr->base_address.access_size
                                    : 1;

    drivers::uart::SerialPort acpi_port(
        hw::IoResource(type, spcr->base_address.address_space_id, stride));
  ...
  if (!s_console.has_value()) [[unlikely]] {
    drivers::uart::SerialPort early_port(
        hw::IoResource(hw::IoType::PMIO, 0x3f8, 1));
