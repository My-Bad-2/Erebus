...
TEST_F(AtoiTest, BasicParsingEquivalence) {
	...
	for (const char *str: test_cases) {
		EXPECT_EQ(klibc::atoi(str), std::atoi(str)) << "Failed on: " << str;
		EXPECT_EQ(klibc::atol(str), std::atol(str)) << "Failed on: " << str;
		EXPECT_EQ(klibc::atoll(str), std::atoll(str)) << "Failed on: " << str;
	}
}
...
TEST_F(AtoiTest, AtollAbsoluteLimits) {
	...
	EXPECT_EQ(klibc::atoll(max_ll), std::numeric_limits<long long>::max());
	EXPECT_EQ(klibc::atoll(min_ll), std::numeric_limits<long long>::min());
}
