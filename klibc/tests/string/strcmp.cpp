...
TEST_F(StringCompareTest, StrcmpDelegatesCorrectly) {
	...
	EXPECT_EQ(klibc::strcmp(s1, s2), 0);

	std::strcpy(s2, "Erebus OS");
	AssertSignMatch(klibc::strcmp(s1, s2), std::strcmp(s1, s2), "s1 > s2");
...
TEST_F(StringCompareTest, ZeroCountReturnsZero) {
	...
	EXPECT_EQ(klibc::strncmp(s1, s2, 0), 0);
}
...
TEST_F(StringCompareTest, DifferenceAtNullByte) {
	...
	int actual = klibc::strncmp(s1, s2, 64);
...
TEST_F(StringCompareTest, MismatchAfterNullByteIsIgnored) {
	...
	int actual = klibc::strncmp(s1, s2, 8);
...
TEST_F(StringCompareTest, PageBoundarySafeGuards) {
	...
	for (const int offset: danger_offsets) {
		...
		EXPECT_EQ(klibc::strncmp(s1, s2, len + 1), 0) << "Exact match failed at page offset " << offset;

		s2[len - 2] = 'X';

		int actual = klibc::strncmp(s1, s2, len + 1);
