...
```cpp
char buf[128];
std::size_t written = klibc::kprint(buf, sizeof(buf), "Hello {}!", "World");
```
...
```cpp
Sink sink;
klibc::format_to(sink, "Invalid address at {:#010x}", fault_addr);
```
...
== Dynamic Widths (`klibc::dyn`)

When column widths must be calculated at runtime, wrap the variable in `klibc::dyn()`. This overrides the format string's width specifier.

```cpp
int max_width = get_longest_name();
klibc::kprint(buf, size, "[{:<}]", klibc::dyn(name, max_width));
```

== ANSI Colors & Styling (`klibc::styled`)
...
When debugging, dumping raw physical or virtual memory is a daily requirement. `kformat` includes a built-in `klibc::hexdump` wrapper that formats arbitary memory blocks instantly.
...
klibc::kprint(buf, size, "Page Content: {:X}",
              klibc::hexdump{page_buffer, sizeof(page_buffer)});
...
To make `kformat` understand a custom kernel struct, specialize the `klibc::formatter<T>` trait in the `klibc` namespace.
...
namespace klibc {
  ...
  struct formatter<CustomStruct> {
    ...
    static constexpr void format(Sink& sink, const CustomStruct& val, const FormatSpec& spec) noexcept {
      ...
      std::size_t len = klibc::kprint(temp, sizeof(temp), "VAL: v={:#x}", val.data);
