...
namespace klib {
namespace {
template <bool AcceptSet>
[[gnu::always_inline]] std::size_t span_impl(const char *dest,
  ...
  for (const unsigned char *s = reinterpret_cast<const unsigned char *>(src);
       *s; ++s) {
    map[*s >> 6] |= (1UL << (*s & 63));
  }
  ...
  while (true) {
    w = string::unaligned_load<std::uint64_t>(
...
size_t strspn(const char *dest, const char *src) noexcept {
  return span_impl<true>(dest, src);
}

size_t strcspn(const char *dest, const char *src) noexcept {
  return span_impl<false>(dest, src);
}
} // namespace klib
