...
namespace klib {
	namespace {
		...
		[[gnu::always_inline]] std::size_t span_impl(const char *dest, const char *src) noexcept {
			using namespace string;
			std::uint64_t map[4] = {0, 0, 0, 0};

			// For strcspn, we must stop at the null terminator. By forcing the \0 bit to be part of the "reject" set, we
			// eliminate the `if (c == '\0`)` branch.
