	...
	size_t strnlen(const char *str, size_t maxlen) noexcept {
		...
		while (processed + 32 <= maxlen) {
			const auto w1 = string::unaligned_load<std::uint64_t>(
					reinterpret_cast<const void *>(current));
			if (const auto m1 = string::has_zero_byte<std::uint64_t>(w1))
					[[unlikely]] {
			...
			if (const auto m2 = string::has_zero_byte<std::uint64_t>(w2))
					[[unlikely]] {
				return processed + 8 +
			...
			if (const auto m3 = string::has_zero_byte<std::uint64_t>(w3))
					[[unlikely]] {
				return processed + 16 +
			...
			if (const auto m4 = string::has_zero_byte<std::uint64_t>(w4))
					[[unlikely]] {
				return processed + 24 +
