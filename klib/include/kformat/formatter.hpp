...
inline constexpr auto kDigitPairs = "0001020304050607080910111213141516171819"
...
template <typename T> struct formatter {
  ...
  static constexpr void format(Sink &buf, const T &,
...
template <> struct formatter<std::string_view> {
  template <typename Sink>
  static constexpr void format(Sink &buf, const std::string_view val,
                               const FormatSpec &spec) noexcept {
    const int len = static_cast<int>(val.length());
    const int pad = (spec.width > len) ? spec.width - len : 0;
    const char align = (spec.align != 0)
                           ? spec.align
                           : '<'; // Strings are left-align by default
...
template <> struct formatter<const char *> {
  template <typename Sink>
  static constexpr void format(Sink &buf, const char *val,
                               const FormatSpec &spec) noexcept {
    formatter<std::string_view>::format(
        buf, val ? std::string_view(val) : "(null)", spec);
  }
};
...
template <> struct formatter<char> {
  template <typename Sink>
  static constexpr void format(Sink &buf, char val,
                               const FormatSpec &spec) noexcept {
    char temp[1] = {val};
...
template <> struct formatter<bool> {
  template <typename Sink>
  static constexpr void format(Sink &buf, bool val,
                               const FormatSpec &spec) noexcept {
    formatter<std::string_view>::format(buf, val ? "true" : "false", spec);
  }
};

template <std::integral T> struct formatter<T> {
  template <typename Sink>
  static constexpr void format(Sink &buf, T val,
                               const FormatSpec &spec) noexcept {
    ...
    const char *hex_digits =
        spec.uppercase ? "0123456789ABCDEF" : "0123456789abcdef";
    ...
    const int total_len =
        num_len + (is_neg ? 1 : 0) + static_cast<int>(prefix.length());
    const int pad = spec.width > total_len ? spec.width - total_len : 0;
    const char align =
        (spec.align != 0) ? spec.align : '>'; // Numbers right-align by default
...
template <> struct formatter<const void *> {
  template <typename Sink>
  static constexpr void format(Sink &buf, const void *ptr,
                               const FormatSpec &spec) noexcept {
    if (!ptr) {
    ...
    formatter<std::uintptr_t>::format(
        buf, reinterpret_cast<std::uintptr_t>(ptr), hex_spec);
  }
};
...
template <> struct formatter<hexdump> {
  template <typename Sink>
  static constexpr void format(Sink &buf, const hexdump &dump,
                               const FormatSpec &spec) noexcept {
...
template <typename T> struct dynamic_width_wrapper {
  const T &value;
  int width;
};
...
template <typename T> struct formatter<dynamic_width_wrapper<T>> {
  template <typename Sink>
  static constexpr void format(Sink &buf, const dynamic_width_wrapper<T> &wrap,
                               FormatSpec spec) noexcept {
...
template <typename T> struct ansi_wrapper {
  const T &value;
  fg::code f;
  bg::code b;
  text_style::code s;
...
template <typename T>
constexpr ansi_wrapper<T>
styled(const T &val, fg::code f, bg::code b = bg::reset,
       text_style::code s = text_style::none) noexcept {
  return {val, f, b, s};
}

template <typename T> struct formatter<ansi_wrapper<T>> {
  template <typename Sink>
  static constexpr void format(Sink &sink, const ansi_wrapper<T> &wrap,
                               FormatSpec spec) noexcept {
    if (wrap.f == fg::reset && wrap.b == bg::reset &&
        wrap.s == text_style::none) {
