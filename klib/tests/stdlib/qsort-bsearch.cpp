...
TEST_F(SearchSortTest, QSortBasicFunctionality) {
	...
	klib::qsort(data.data(), data.size(), sizeof(int), cmp_int);
...
TEST_F(SearchSortTest, QSortWeirdStructSizes) {
	...
	klib::qsort(data.data(), data.size(), sizeof(WeirdStruct), cmp_struct);
...
TEST_F(SearchSortTest, BsearchBasicFunctionality) {
	...
	int key = 30;
	void *result = klib::bsearch(&key, sorted_data.data(), sorted_data.size(), sizeof(int), cmp_int);
	ASSERT_NE(result, nullptr);
	EXPECT_EQ(*static_cast<int *>(result), 30);

	key = 35;
	result = klib::bsearch(&key, sorted_data.data(), sorted_data.size(), sizeof(int), cmp_int);
...
TEST_F(SearchSortTest, BsearchExponentialBoundsGuarantees) {
	...
	int first_key = 0;
	void *res1 = klib::bsearch(&first_key, sorted_data.data(), sorted_data.size(), sizeof(int), cmp_int);
	ASSERT_NE(res1, nullptr);
	EXPECT_EQ(*static_cast<int *>(res1), 0);

	int last_key = 1998;
	void *res2 = klib::bsearch(&last_key, sorted_data.data(), sorted_data.size(), sizeof(int), cmp_int);
	ASSERT_NE(res2, nullptr);
	EXPECT_EQ(*static_cast<int *>(res2), 1998);

	int missing_beyond = 3000;
	void *res3 = klib::bsearch(&missing_beyond, sorted_data.data(), sorted_data.size(), sizeof(int), cmp_int);
