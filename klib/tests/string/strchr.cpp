...
TEST_F(StringSearchTest, StrchrBasicFind) {
	...
	EXPECT_EQ(klib::strchr(str, 'b'), str + 3);
	EXPECT_EQ(klib::strchr(str, 'K'), str + 10);
	EXPECT_EQ(klib::strchr(str, 'Z'), nullptr);
}
...
TEST_F(StringSearchTest, StrchrNullTerminatorTarget) {
	...
	EXPECT_EQ(klib::strchr(str, '\0'), str + 6);
}
...
TEST_F(StringSearchTest, StrchrPrecedingGarbageTrap) {
	...
	aligned_base[1] = 's';
	EXPECT_EQ(klib::strchr(str, 's'), str + 2);
}
...
TEST_F(StringSearchTest, StrrchrBasicFind) {
	...
	EXPECT_EQ(klib::strrchr(str, 'e'), str + 14);
	EXPECT_EQ(klib::strrchr(str, 'E'), str + 0);
	EXPECT_EQ(klib::strrchr(str, 'O'), str + 7);
	EXPECT_EQ(klib::strrchr(str, 'Z'), nullptr);
}
...
TEST_F(StringSearchTest, StrrchrNullTerminatorTarget) {
	...
	EXPECT_EQ(klib::strrchr(str, '\0'), str + 6);
}
	...
	for (const std::size_t len: lengths) {
		for (const std::size_t align: alignments) {
			...
			if (len >= 2) {
				...
				EXPECT_EQ(klib::strchr(str, 'T'), str + 0) << "strchr failed at len: " << len << ", align: " << align;

				EXPECT_EQ(klib::strrchr(str, 'T'), str + (len - 1))
