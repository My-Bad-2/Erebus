...
TEST_F(AdvancedStringSearchTest, StrpbrkBasicMatch) {
	...
	EXPECT_EQ(klib::strpbrk(str, "zxm"), str + 5);

	// 'I' is the first character that matches "I"
	EXPECT_EQ(klib::strpbrk(str, "I"), str + 7);
}
...
TEST_F(AdvancedStringSearchTest, StrpbrkNoMatchNullTrap) {
	...
	EXPECT_EQ(klib::strpbrk(str, "Z"), nullptr);
	EXPECT_EQ(klib::strpbrk(str, ""), nullptr);
}
...
TEST_F(AdvancedStringSearchTest, StrstrBasicMatch) {
	...
	EXPECT_EQ(klib::strstr(str, "OS"), str + 7);
	EXPECT_EQ(klib::strstr(str, "Kernel"), str + 10);
	EXPECT_EQ(klib::strstr(str, "Erebus"), str + 0);
}
...
TEST_F(AdvancedStringSearchTest, StrstrLargerNeedleRejection) {
	...
	EXPECT_EQ(klib::strstr(str, "ShortString"), nullptr);
}
...
TEST_F(AdvancedStringSearchTest, StrstrTheMississippiTrap) {
	...
	EXPECT_EQ(klib::strstr(str, "issip"), str + 4); // "issip" is at index 4
	EXPECT_EQ(klib::strstr(str, "issi"), str + 1); // The first "issi" is at index 1

	std::strcpy(str, "AAAAAAAAAAAAAAAAAAAAAB");
	EXPECT_EQ(klib::strstr(str, "AAAAAB"), str + 16);
}
...
TEST_F(AdvancedStringSearchTest, StrstrBoundsTrap) {
	...
	EXPECT_EQ(klib::strstr(str, "StringX"), nullptr);
}
