...
TEST_F(StringSpanTest, StrspnBasicMatch) {
	...
	EXPECT_EQ(klib::strspn(str, "0123456789"), 3); // "742"
	EXPECT_EQ(klib::strspn(str, " 742"), 4); // "742 "

	EXPECT_EQ(klib::strspn(str, "Evergreen"), 0);
}
...
TEST_F(StringSpanTest, StrspnEmptyEdgeCases) {
	...
	EXPECT_EQ(klib::strspn(str, ""), 0); // Empty accept set
	EXPECT_EQ(klib::strspn("", "Kernel"), 0); // Empty dest string
}
...
TEST_F(StringSpanTest, StrcspnBasicReject) {
	...
	EXPECT_EQ(klib::strcspn(str, "."), 6); // stops at '.'
	EXPECT_EQ(klib::strcspn(str, "f"), 10); // stops at 'f' in config

	EXPECT_EQ(klib::strcspn(str, "Z"), 17);
}
...
TEST_F(StringSpanTest, StrcspnEmptyEdgeCases) {
	...
	EXPECT_EQ(klib::strcspn(str, ""), 6);
	EXPECT_EQ(klib::strcspn("", "Kernel"), 0);
}
...
TEST_F(StringSpanTest, ExhaustiveSizesAndAlignments) {
	...
	for (const std::size_t len: lengths) {
		for (const std::size_t align: alignments) {
			...
			EXPECT_EQ(klib::strspn(str, "A"), len) << "strspn failed full match";
			EXPECT_EQ(klib::strcspn(str, "B"), len) << "strcspn failed full bypass";

			if (len >= 2) {
				const std::size_t inject_idx = len / 2;

				str[inject_idx] = 'X';
				EXPECT_EQ(klib::strspn(str, "AB"), inject_idx);
				str[inject_idx] = 'A';

				str[inject_idx] = 'Y';
				EXPECT_EQ(klib::strcspn(str, "YZ"), inject_idx);
			}
		}
	}
}
