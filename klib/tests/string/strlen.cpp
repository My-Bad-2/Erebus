...
TEST_F(StrlenTest, PrecedingNullByteTrap) {
	...
	EXPECT_EQ(klib::strlen(str), 5) << "The down-alignment logic failed to ignore preceding null bytes!";
}
...
TEST_F(StrlenTest, ExhaustiveSizesAndAlignments) {
	...
	for (const std::size_t len: lengths) {
		for (const std::size_t align: alignments) {
			...
			std::size_t const actual_len = klib::strlen(str);
...
TEST_F(StrnlenTest, ZeroMaxlenReturnsZero) {
	...
	EXPECT_EQ(klib::strnlen(str, 0), 0);
}
...
TEST_F(StrnlenTest, PrecedingNullByteTrapWithMaxlen) {
	...
	EXPECT_EQ(klib::strnlen(str, 2), 2);
	EXPECT_EQ(klib::strnlen(str, 10), 5);
}
	...
	for (const std::size_t len: lengths) {
		for (const std::size_t align: alignments) {
			...
			for (const std::size_t maxlen: maxlens) {
				std::size_t const actual_len = klib::strnlen(str, maxlen);
